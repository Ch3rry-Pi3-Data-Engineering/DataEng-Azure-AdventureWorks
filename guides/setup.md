...
- For Synapse, set `SYNAPSE_AAD_ADMIN_LOGIN` (group recommended) in `.env`. The deploy script resolves the object id when possible; if directory reads are restricted, also set `SYNAPSE_AAD_ADMIN_OBJECT_ID` or add `aad_admin_object_id` to `terraform/11_synapse_analytics/terraform.tfvars`. The deploy script generates a SQL admin password if missing (per-user); you can override with `SYNAPSE_SQL_ADMIN_PASSWORD`. Optionally set `SYNAPSE_SQL_ADMIN_LOGIN` and `SYNAPSE_FILESYSTEM_NAME` to override defaults.
